...
export const GetEmployeeList = async () => {
    try {
        const response = await axios.get("http://localhost:8080/api/employee/getlist")
...
export const UpdateEmployee = async (formdata,id) => {
    try {
        const response = await axios.put(`http://localhost:8080/api/employee/update/${id}`, formdata, {
            headers: {
                "Authorization": `Bearer ${localStorage.getItem("token")}`
            }
        })
