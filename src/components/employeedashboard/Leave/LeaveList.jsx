          ...
          <tbody className="divide-y divide-gray-200">
            {leaveData.map((leave,index) => (
              <tr key={leave.id} className="hover:bg-gray-50">
                ...
                <td className="py-3 px-4 text-sm text-gray-600">{new Date(leave.startDate).toLocaleDateString("en-GB")}</td>
                <td className="py-3 px-4 text-sm text-gray-600">{new Date(leave.endDate).toLocaleDateString("en-GB")}</td>
                <td className="py-3 px-4 text-sm text-gray-600">{leave.reason}</td>
                <td className="py-3 px-4 text-sm text-gray-600">{new Date(leave.appliedAt).toLocaleDateString("en-GB")}</td>
                <td className="py-3 px-4 text-sm text-gray-600">{leave.status}</td>
              </tr>
            ))}
          </tbody>
        </table>
